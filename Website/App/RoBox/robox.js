...
function roboxInit()
{
    ...
    // currently, this starts at the first challenge, it should check cookies instead
    roboxBlocklyChallengeLoad(challenges[0]);
    roboxUIChallengeLoad(challenges[0]);
...
function roboxCurrentChallenge()
{
    var currentChallenge = $('#topDivTitle .number').text();
...
function roboxNextPrevButtonLightUp(c)
{
...
{
    var cur = roboxCurrentChallenge();

    if(cur) {
	var i = challenges.indexOf(cur);
	if(i !== challenges.length-1) {
	    var next = challenges[i+1];
	    roboxBlocklyChallengeLoad(next);
	    roboxUIChallengeLoad(next);
	}
    }
...
function roboxUIChallengeLoad(id)
{
    // if bad challenge, it is ignored
    
    if(!challenges.includes(id)) {
	console.log("Bad challenge for roboxUIChallengeLoad - " + id);
	return;
    }
    ...
    roboxNextPrevButtonLightUp(challenges.indexOf(id))
    roboxExplain();
}
...
function saveWorkspace() {
    ...
    localStorage.setItem("blockly.xml" + url.charAt(url.length-2), xmlText);
}

function loadWorkspace() {
    var url = window.location.href;
    var xmlText = localStorage.getItem("blockly.xml" + url.charAt(url.length-2));
    if (xmlText) {
...
function roboxSave(challenge)
{
    saveWorkspace()
}

function roboxLoad(challenge)
{
    loadWorkspace()
}
